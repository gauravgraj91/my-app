...
describe('BillCreateModal', () => {
  ...
  describe('Rendering', () => {
    it('renders modal when open', () => {
      ...
      expect(screen.getByLabelText(/Total Amount Paid/)).toBeInTheDocument();
    });
  ...
  describe('Product Details & Calculations', () => {
    it('calculates cost, profit and total profit correctly', async () => {
      render(<BillCreateModal {...defaultProps} />);

      const mrpInput = screen.getByLabelText(/MRP/);
      const quantityInput = screen.getByLabelText(/Quantity/);
      const totalAmountInput = screen.getByLabelText(/Total Amount Paid/);

      await userEvent.type(mrpInput, '100');
      await userEvent.type(quantityInput, '10');
      await userEvent.type(totalAmountInput, '800');

      // Cost per Unit = 800 / 10 = 80
      // Profit per Piece = 100 - 80 = 20
      // Total Profit = 20 * 10 = 200

      expect(screen.getByText('₹80.00')).toBeInTheDocument(); // Cost per Unit
      expect(screen.getByText('₹20.00')).toBeInTheDocument(); // Profit per Piece
    ...
    it('handles zero quantity gracefully', async () => {
      ...
      const costDisplays = screen.getAllByText('₹0.00');
