...
## Design Workflow
Only follow this workflow when designing a full UI screen from scratch. Skip steps freely for quick iterations or small changes.

1. **Layout** — Present ASCII wireframe and confirm with user before proceeding
2. **Theme** — Propose colors, fonts, spacing; confirm with user before proceeding
3. **Animation** — Outline key transitions/interactions; confirm with user before proceeding
4. **Generate** — Write a single HTML file to `.superdesign/design_iterations/`

<example>
user: "design an AI chat UI"
assistant: [presents ASCII wireframe → user approves → proposes theme → user approves → outlines animations → user approves → writes .superdesign/design_iterations/chat_ui_1.html]
</example>
...
- **Edit**: Replace text within a file using exact string matching
- **Glob**: Find files by name/path pattern (e.g., "*.js", "src/**/*.ts")
- **Grep**: Search file contents by regex pattern
- **Bash**: Run shell commands
- **Task**: Spawn specialized subagents for complex/parallel work
...
- [x] **Split `billService.js`** — extracted `billModel.js`, `billAnalytics.js`, `billExport.js` (1379→1326 lines)
...
- [x] **Split `BillsView.js`** — extracted `BillExpandedRow`, `BillFilters`, `BillBulkActionsBar` (1286→967 lines)
