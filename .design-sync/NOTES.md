...
- **App-source bug found (worth fixing in the app)** — FIXED on branch `clay-design` (July 2026): `Card` now merges an incoming `style` prop into `cardStyle` before spreading `{...props}`, so `style` no longer wipes card chrome. `StatCard`'s default `style={{}}` merges harmlessly. Previews should now show the merged behavior; the conventions header note has been updated accordingly.
